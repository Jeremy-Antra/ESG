...
def generate_input_df():
    data = {
        ...
        'S5: Community engagement and philanthropic activities': [random.randint(1, 100) for _ in
                                                                  range(num_of_company)],
        ...
        'G5: Ethical business conduct and anti-corruption policies': [random.randint(1, 100) for _ in
                                                                      range(num_of_company)],
...
st.write("""
# ESG For Everyone - Challenge 1
""")

st.write("""
## Enter Your Company ESG Info For Suggestions
""")
# col1, col2 = st.columns([1, 3])
# with col1:
#     generate = st.button('Generate Data', on_click=delete_input())
# with col2:
#     clear = st.button('Empty Form', on_click=delete_input())

inputs = {}

# if generate:
#     gen_df = generate_input_df()
#     kv = gen_df.to_dict(orient='records')[0].items()
#     with st.form('geninput'):
#         for k, v in kv:
#             inputs[k] = [st.text_input(k, v)]
...
with st.form('userinput'):
    ...
    if submitted:
        ...
        st.dataframe(df_input, top_bottom, suggest)
